...
void zCollision(){
	...
	if (currLevel == 1){
		...
		zc1 = (xBullet >= z01_X - 5 && xBullet <= z01_X+58 && yBullet >= 113 && yBullet <= 196 && z[0].health > 0);
		zc2 = (xBullet >= z02_X - 5 && xBullet <= z02_X+109 && yBullet >= 86 && yBullet <= 219 && z[1].health > 0);
		zc3 = (xBullet >= z03X + 13 && xBullet <= z03X+87 && yBullet >= 321 && yBullet <= 426 && z[2].health > 0);
	}
	else if (currLevel == 3){
		zc1 = (xBullet >= z01_X + 13 && xBullet <= z01_X + 68 && yBullet >= 423 && yBullet <= 486 && z[0].health > 0);
		zc2 = (xBullet >= z02_X + 16 && xBullet <= z02_X + 102 && yBullet >= 86 && yBullet <= 219 && z[1].health > 0);
		zc3 = (xBullet >= z03X+8 && xBullet <=  z03X+90 && yBullet >= 415 && yBullet <= 505 && z[2].health > 0);
	}
...
void loading(){
	...
	iSetColor(r, g, b);
	iText((screenWidth/2)-70 , 140, name, GLUT_BITMAP_TIMES_ROMAN_24);
	r = rand() % 255;
	g = rand() % 255;
	b = rand() % 255;
...
void About(){
	...
	iSetColor(255, 255, 0);
	iText(screenWidth / 2 - 160, 40, "Press Left Mouse Button to Back", GLUT_BITMAP_TIMES_ROMAN_24);
}
void level_one(){
...
void level_two(){
	...
	if (z[0].health == 3)
		iShowBMP2(z01_X, z01_Y, z03[zindex], 0);
	else if (z[0].health == 2)
		iShowBMP2(z01_X, z01_Y, z02[zindex], 0);
	else if (z[0].health == 1)
		iShowBMP2(z01_X, z01_Y, z01[zindex], 0);
	if (z[1].health == 2)
		iShowBMP2(z02_X, z02_Y, z12[zindex], 0);
	else if (z[1].health == 1)
		iShowBMP2(z02_X, z02_Y, z11[zindex], 0);
	if (z[2].health == 1){
		iShowBMP2(z03X, z03Y, z2[zindex], 0);
	}
	zombiePositionChange_level2();
}
...
{	
	...
	PlaySound("music\\bgm", NULL, SND_LOOP | SND_ASYNC);
	//return 0;
	iInitialize(screenWidth, screenHeight, "Graveyard");
	iStart();
	return 0;
}